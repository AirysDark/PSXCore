...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val lastDeviceAddress: StateFlow<String?> =
        settingsRepository.lastDeviceAddress
            .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), null)

    val lastDeviceName: StateFlow<String?> =
        settingsRepository.lastDeviceName
            .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), null)
    ...
    private fun sendControlCommand(
        ...
        synchronized(commandLock) {
            ...
            if (previous != null && now - previous < cooldownMs) {
                Log.d(
                    tag,
                    "[BLE] Duplicate command suppressed: " +
                            "$command (${now - previous}ms since previous)"
                )
                return false
            }
    ...
    fun connectToDevice(device: BleDevice) {
        ...
        viewModelScope.launch {
            settingsRepository.saveLastDevice(
                device.address,
                device.name
            )
        }
    ...
    fun sendPing(): Boolean =
        sendControlCommand(ProtocolConstants.CMD_PING, 350)

    fun sendGetInfo(): Boolean =
        sendControlCommand(ProtocolConstants.CMD_INFO, 500)

    fun sendGetState(): Boolean =
        sendControlCommand(ProtocolConstants.CMD_GET_STATE, 350)

    fun sendGetSettings(): Boolean =
        sendControlCommand(ProtocolConstants.CMD_GET_SETTINGS, 500)

    fun setAnalogMode(): Boolean =
        sendControlCommand(ProtocolConstants.CMD_SET_ANALOG, 1000)

    fun requestOtaInfo(): Boolean =
        sendControlCommand(ProtocolConstants.CMD_OTA_INFO, 500)
