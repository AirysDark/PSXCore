...
static bool finishOta() {
  ...
  if(otaReceivedSize!=otaExpectedSize){
    Serial.printf("[OTA] SIZE MISMATCH: received=%lu expected=%lu\n",(unsigned long)otaReceivedSize,(unsigned long)otaExpectedSize);
    abortOta("size_mismatch"); return false;
  }
  Serial.println("[OTA] FINALIZING IMAGE...");
  ...
  bleConfigSendText("{\"type\":\"ota\",\"state\":\"success\",\"progress\":100,\"rebooting\":true}\n");
  Serial.println("[OTA] SUCCESS - image validated and boot partition switched");
  Serial.println("[OTA] REBOOTING IN 1500 ms");
  return true;
}

static int16_t psxAxisToHid(uint8_t v){return(int16_t)((uint32_t(v)*32767U+127U)/255U);}
static void sendConfigHello(){bleConfigSendText("{\"type\":\"hello\",\"device\":\"PSXCore\",\"protocol\":4,\"transport\":\"ble-nus\",\"hid\":true,\"liveState\":true,\"ota\":\"ready\",\"otaSupported\":true,\"otaChunkTransport\":\"raw\",\"otaEndCommand\":\"OTA_END\"}\n");}
static void sendInfo(){bleConfigSendText("{\"type\":\"info\",\"device\":\"PSXCore\",\"protocol\":4,\"hid\":true,\"config\":true,\"liveState\":true,\"ota\":\"ready\",\"otaSupported\":true,\"otaChunkTransport\":\"raw\",\"otaEndCommand\":\"OTA_END\"}\n");}
