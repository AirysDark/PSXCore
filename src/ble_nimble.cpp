...
static void resetOtaState() { otaState=OtaState::Idle; otaExpectedSize=0; otaReceivedSize=0; otaProgress=0; otaLastError=ESP_OK; otaLastReportedProgress=255; }
static void abortOta(const char* reason) {
  ...
  char m[192]; snprintf(m,sizeof(m),"{\"type\":\"ota\",\"state\":\"error\",\"reason\":\"%s\",\"error\":%u}\n",reason?reason:"unknown",Update.getError()); bleConfigSendText(m);
}
static void sendOtaInfo() {
  char m[256]; snprintf(m,sizeof(m),"{\"type\":\"ota\",\"supported\":true,\"state\":\"%s\",\"expectedSize\":%lu,\"receivedSize\":%lu,\"progress\":%u,\"availableSpace\":%lu,\"updateInProgress\":%s,\"error\":%d}\n",otaStateName(otaState),(unsigned long)otaExpectedSize,(unsigned long)otaReceivedSize,otaProgress,(unsigned long)otaAvailableSpace(),Update.isRunning()?"true":"false",(int)otaLastError); bleConfigSendText(m);
}
...
// PART 3: Receive raw BLE firmware chunks and stream them directly into Update.write().
static bool writeOtaChunk(const uint8_t* data,size_t length) {
  ...
  if(written!=length){
    ...
    Serial.printf("[OTA] WRITE FAILED: wanted=%lu wrote=%lu total=%lu/%lu\n",(unsigned long)length,(unsigned long)written,(unsigned long)otaReceivedSize,(unsigned long)otaExpectedSize);
    abortOta("write_failed"); return false;
  }
  otaReceivedSize+=written;
  Serial.printf("[OTA] CHUNK: +%lu bytes, total=%lu/%lu (%u%%)\n",(unsigned long)written,(unsigned long)otaReceivedSize,(unsigned long)otaExpectedSize,(unsigned)((otaReceivedSize*100ULL)/otaExpectedSize));
  sendOtaProgress(false);
  return true;
}

static int16_t psxAxisToHid(uint8_t v){return(int16_t)((uint32_t(v)*32767U+127U)/255U);}
static void sendConfigHello(){bleConfigSendText("{\"type\":\"hello\",\"device\":\"PSXCore\",\"protocol\":3,\"transport\":\"ble-nus\",\"hid\":true,\"liveState\":true,\"ota\":\"ready\",\"otaSupported\":true,\"otaChunkTransport\":\"raw\"}\n");}
static void sendInfo(){bleConfigSendText("{\"type\":\"info\",\"device\":\"PSXCore\",\"protocol\":3,\"hid\":true,\"config\":true,\"liveState\":true,\"ota\":\"ready\",\"otaSupported\":true,\"otaChunkTransport\":\"raw\"}\n");}
static void sendSettings(){bleConfigSendText("{\"type\":\"settings\",\"analogControl\":true,\"sleepMinutes\":5}\n");}

static bool isLikelyTextCommand(const uint8_t* data,size_t length) {
  if(!data||!length)return false;
  size_t n=length<16?length:16;
  for(size_t i=0;i<n;i++){uint8_t c=data[i]; if(c==0)return false; if(c<0x20||c>0x7E){if(c!='\n'&&c!='\r'&&c!='\t')return false;}}
  return true;
}
...
static void onConfigData(const uint8_t* data,size_t length) {
  ...
  // Once OTA_BEGIN succeeds, every non-text BLE payload is firmware data.
  if(otaState==OtaState::Receiving && Update.isRunning() && !isLikelyTextCommand(data,length)) { writeOtaChunk(data,length); return; }
...
void ble_init(){BleGamepadConfiguration c;c.setAutoReport(false);c.setButtonCount(16);c.setHatSwitchCount(1);resetOtaState();bleGamepad.begin(&c);bleGamepad.beginNUS();bleGamepad.setNUSDataReceivedCallback(onConfigData);configReady=(bleGamepad.getNUS()!=nullptr);Serial.println("[BLE] HID service: READY");Serial.printf("[BLE] Android config service: %s\n",configReady?"READY":"FAILED");Serial.printf("[OTA] Part 3 chunk receiver: %s, next slot=%lu bytes\n",configReady?"READY":"WAITING",(unsigned long)otaAvailableSpace());}
void ble_send_report(){bool connected=bleGamepad.isConnected();debugStatusBLEState(connected);if(connected!=lastConnected){Serial.printf("[BLE] HID %s\n",connected?"CONNECTED":"DISCONNECTED");lastConnected=connected;if(connected&&configReady)sendConfigHello();}if(!connected)return;updatePsxButtons(controllerState.buttons);bleGamepad.setLeftThumb(psxAxisToHid(controllerState.lx),psxAxisToHid(controllerState.ly));bleGamepad.setRightThumb(psxAxisToHid(controllerState.rx),psxAxisToHid(controllerState.ry));bleGamepad.sendReport();debugStatusBLEUpdate();}
void bleGamepadBegin(){ble_init();debugStatusBLEState(false);}void bleGamepadUpdate(){ble_send_report();}
bool bleConfigIsReady(){return configReady;}void bleConfigSend(const uint8_t*d,size_t l){if(configReady&&d&&l)bleGamepad.sendDataOverNUS(d,l);}void bleConfigSendText(const char*t){if(t)bleConfigSend(reinterpret_cast<const uint8_t*>(t),strlen(t));}
void bleConfigNotifyControllerState(){if(!configReady)return;const ControllerState&s=controllerState;bool changed=s.buttons!=lastStateButtons||s.lx!=lastStateLx||s.ly!=lastStateLy||s.rx!=lastStateRx||s.ry!=lastStateRy;if(!changed)return;char m[160];snprintf(m,sizeof(m),"{\"type\":\"state\",\"buttons\":%lu,\"lx\":%u,\"ly\":%u,\"rx\":%u,\"ry\":%u}\n",(unsigned long)s.buttons,s.lx,s.ly,s.rx,s.ry);bleConfigSendText(m);lastStateButtons=s.buttons;lastStateLx=s.lx;lastStateLy=s.ly;lastStateRx=s.rx;lastStateRy=s.ry;}
