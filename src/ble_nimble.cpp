...
static void sendConfigHello(){bleConfigSendText("{\"type\":\"hello\",\"device\":\"PSXCore\",\"protocol\":5,\"transport\":\"psxcore-gatt\",\"hid\":true,\"liveState\":true,\"ota\":\"ready\",\"otaSupported\":true,\"otaChunkTransport\":\"ota-data-characteristic\",\"otaEndCommand\":\"OTA_END\"}\n");}
static void sendInfo(){bleConfigSendText("{\"type\":\"info\",\"device\":\"PSXCore\",\"protocol\":5,\"transport\":\"psxcore-gatt\",\"hid\":true,\"config\":true,\"liveState\":true,\"ota\":\"ready\",\"otaSupported\":true,\"otaChunkTransport\":\"ota-data-characteristic\",\"otaEndCommand\":\"OTA_END\"}\n");}
...
static bool isLikelyTextCommand(const uint8_t* data,size_t length) {
  if(!data||!length)return false; size_t n=length<32?length:32;
  for(size_t i=0;i<n;i++){uint8_t c=data[i]; if(c==0)return false; if(c<0x20||c>0x7E){if(c!='\n'&&c!='\r'&&c!='\t')return false;}} return true;
}

static void onGattData(const uint8_t* data,size_t length) {
...
void ble_init(){BleGamepadConfiguration c;c.setAutoReport(false);c.setButtonCount(16);c.setHatSwitchCount(1);resetOtaState();bleGamepad.begin(&c);configReady=psxCoreGattBegin(onGattData);Serial.println("[BLE] HID service: READY");Serial.printf("[BLE] PSXCore custom GATT: %s\n",configReady?"READY":"FAILED");Serial.printf("[OTA] Custom GATT OTA: %s, next slot=%lu bytes\n",configReady?"READY":"WAITING",(unsigned long)otaAvailableSpace());}
